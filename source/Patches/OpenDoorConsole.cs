...
namespace TownOfUs
{
    ...
    public class OpenDoorConsoleCanUse
    {
            ...
            ref bool __state)
        {
            ...
            if ((playerControl.Is(RoleEnum.ª√”∞) && !Role.GetRole<Phantom>(playerControl).Caught) || (playerControl.Is(RoleEnum.‘©ªÍ) && !Role.GetRole<Haunter>(playerControl).Caught) && playerInfo.IsDead)
            {
